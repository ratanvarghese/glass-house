...
local function simple_power_list(targ_max, targ_kind, skip_target)
	...
	for i=1,len do
		...
		elseif dice == 3 then
			table.insert(res, {kind=targ_kind+1, factor=targ_max})
		elseif dice == 4 then
...
property "power.max_factor: find max" {
	generators = { int(), int() },
...
property "power.max_factor: return falsy if target kind not found" {
	generators = { int(), int() },
	check = function(targ_max, targ_kind)
		local list = simple_power_list(targ_max, targ_kind, true)
		return not power.max_factor(list, targ_kind)
	end
}

property "power.max_factor: return falsy if input list nil" {
	generators = { int() },
	check = function(targ_kind)
		return not power.max_factor(nil, targ_kind)
	end
}
