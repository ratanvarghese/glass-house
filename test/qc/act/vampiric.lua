...
property "act[enum.power.vampiric].melee: utility" {
	...
	check = function(seed, pos)
		local w, src, targ = melee_setup(seed, pos)
		local m_res = act[enum.power.mundane].melee.utility(w, src, targ.pos)
		local v_res = act[enum.power.vampiric].melee.utility(w, src, targ.pos)
		if m_res == 0 then
			return v_res == 0
		else
			return v_res > m_res
...
property "act[enum.power.vampiric].melee: attempt" {
	...
	check = function(seed, pos)
		...
		if act[enum.power.vampiric].melee.attempt(w, src, targ.pos) then
