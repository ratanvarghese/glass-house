...
function tool_s.process(system, e, dt)
	local world = system.world
	local t = world.terrain[e.pos]
	if e.usetool.pickup and tool_s.has_inventory_i(t, 1) then
		process_pickup(world, e, t)
	end

	if e.usetool.drop and tool_s.has_inventory_i(e, 1) then
		process_drop(world, e, t)
	end

	if e.usetool.equip and tool_s.has_inventory_i(e, 1) then
		process_equip(e)
	end
end

function tool_s.make_system()
	local system = tiny.processingSystem()
	system.filter = tiny.requireAll("inventory", "usetool")
	system.process = tool_s.process
	return system
end
